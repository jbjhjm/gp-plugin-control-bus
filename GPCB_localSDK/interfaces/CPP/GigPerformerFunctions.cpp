...
#define then
   ...
   std::string GigPerformerFunctions::getPluginParameterText(const std::string & pluginHandle, int parameterIndex, bool useGlobalRackspace)
   {
        const int bufferLength = 1024;
        char returnBuffer[bufferLength] = {0};
   ...
   std::string GigPerformerFunctions::getPluginCaption(const std::string & pluginHandle,  bool useGlobalRackspace)
   {
        ...
        /* int actualLength = */ GP_GetPluginCaption(fHandle, pluginHandle.c_str(), returnBuffer, bufferLength, useGlobalRackspace);
        std::string result(returnBuffer);
        return result;
   }
  ...
  std::string GigPerformerFunctions::getPluginName(const std::string & pluginHandle,  bool useGlobalRackspace)
   {
        ...
        /* int actualLength = */ GP_GetPluginName(fHandle, pluginHandle.c_str(), returnBuffer, bufferLength, useGlobalRackspace);
        std::string result(returnBuffer);
        return result;
   }
...
bool GigPerformerFunctions::loadGPPreset(const std::string & pluginHandle, const std::string & presetName, bool useGlobalRackspace)
{
    ...
    if (GP_LoadGPPreset != nullptr)
       then ok = GP_LoadGPPreset(fHandle, pluginHandle.c_str(), presetName.c_str(), useGlobalRackspace);
...
{
   double result = GP_GetSongTempo(fHandle, atIndex);
   return result;
   ...
   std::string GigPerformerFunctions::getVariationNameForSongPart(int atSongIndex, int atPartIndex)
   {
       const int bufferLength = 1024;
       char returnBuffer[bufferLength] = {0};
       /* int actualLength = */ GP_GetVariationNameForSongPart(fHandle,atSongIndex,  atPartIndex,  returnBuffer,  bufferLength);
       std::string result(returnBuffer);
