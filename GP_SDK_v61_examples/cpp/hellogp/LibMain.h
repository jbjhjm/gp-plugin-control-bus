...
public:
    ...
    void OnSetlistChanged(const std::string & newSetlistName) override
    {
        ...
        consoleLog("Setlist count = " + std::to_string(getSetlistCount()));

        int currentIndex = getCurrentSetlistIndex();
        consoleLog("Index: " + std::to_string(currentIndex));
        std::string s = getSetlistName(currentIndex);
        consoleLog("new Setlist name: " + s);
    }
    ...
    void OnSwitchToPanelView() override
    {
        ...
        p1 = getPluginCaption("Legend", false);
        consoleLog(p1);
        p1 = getPluginName("Legend", false);
        consoleLog(p1);
    }
   ...
   void OnTempoChanged(double newBPM) override
   {
       consoleLog("Tempo changed");
       loadGPPreset("Legend", "Gnarl", false);
   }
           ...
           then
              {
                 ...
                 std::string key = getSongKey(0);
                 consoleLog(key);
                 double tempo = getSongTempo(0);
                 consoleLog(std::to_string(tempo));
              }
        /*
